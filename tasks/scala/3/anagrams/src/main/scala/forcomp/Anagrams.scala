...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    List() :: (for {
      ...
      ost <- combinations(occurrences.filter(p => p._1 > occurrence._1))
    } yield List((occurrence._1, count)) ++ ost)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match {
    case Nil => x
    case (v, n) :: tail => {
      val e = x.indexWhere(_._1 == v)
      if (x(e)._2 > n) subtract(x.updated(e, (x(e)._1, x(e)._2 - n)), tail)
      else subtract(x.filter(_._1 != v), tail)
    }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
	def findAnagrams(occurrences: Occurrences): List[Sentence] = {
		if (occurrences.isEmpty) List(Nil)
			...
			sentence <- findAnagrams(subtract(occurrences,wordOccurrences(w)))
