...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case head :: tail =>
      ...
      val res = List(Nil) ++ headOccurs ++ tailOccurs ++ headOccurs.flatMap(h => tailOccurs.map(t => h ++ t))
      res.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val mappedX = x.toMap
    val resultMap = mappedX ++ y.toMap.map{
      case (ch, i) => ch -> (mappedX.getOrElse(ch, 0) - i)
    }
