...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
      (
         for 
           {
             a <- w.toLowerCase.toCharArray.groupBy(c => c).toList
           }
             yield (a._1, a._2.length)
      )
      .sortBy(_._1)
  ...
  {
    def loopAnagrams(occ: Occurrences): List[Sentence] =
    {
      if (occ.isEmpty) List(Nil)
        else for
        {
          ...
          s <- loopAnagrams(subtract(occ, wordOccurrences(w)))
          if comb.nonEmpty
        }
        yield w :: s
    }

    loopAnagrams(sentenceOccurrences(sentence))
  }
