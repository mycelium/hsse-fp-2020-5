...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toSeq.groupBy(x => x.toLower).view.mapValues(_.unwrap).map(x =>(x._1,x._2.length)).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
	def searchAnagrams(occurrences: Occurrences): List[Sentence] = {
		if (occurrences.isEmpty) List(Nil)
		else for {
			combination <- combinations(occurrences)
			word <- dictionaryByOccurrences getOrElse(combination,Nil)
			sentence <- searchAnagrams(subtract(occurrences,wordOccurrences(word)))
			if combination.nonEmpty
		}
		yield word :: sentence
	}
    searchAnagrams(sentenceOccurrences(sentence))
  }
