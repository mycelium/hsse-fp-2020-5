...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toSeq
                                               .groupBy(x => x.toLower)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
	...
	case case Nil => List(Nil)
    case head :: tail =>
        val OccurrencesHead = combinations(List(head))
        val OccurrencesTail = combinations(tail)
        val result = List(Nil) ++ OccurrencesHead ++ OccurrencesTail ++ OccurrencesHead.flatMap(oh => OccurrencesTail.map(ot => oh ++ ot))
        result.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val MapX = x.toMap
    val resultMap = MapX ++ y.toMap.map{
      case (c, index) => c -> (MapX.getOrElse(c, 0) - index)
    }
