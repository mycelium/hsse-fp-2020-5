...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = s.map(word => wordOccurrences(word)).reduceOption((left, right) => (left ++ right).groupBy(_._1).mapValues(_.map(_._2).sum).toList).getOrElse(List()).sortWith(_._1 < _._1)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    val sentences = addSentence(level)
    sentences
  }
      ...
      if (comb != Nil) {
        ...
        if (words.nonEmpty) {
          ...
          if (subtractedOccurrences.nonEmpty) {
            ...
            if (leafs != null) level = (words, leafs.asInstanceOf[TreeLevel])
  ...
  def addSentence(level: TreeLevel): List[Sentence] = {
    level.flatMap(elem => { if (elem._2 != null) { elem._1.flatMap(word => addSentence(elem._2.asInstanceOf[TreeLevel]).map(_.::(word))) } else { elem._1.map(word => List(word)) } })
  }
}
