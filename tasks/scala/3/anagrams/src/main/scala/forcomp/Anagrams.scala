...
object Anagrams {
  ...
  def calcCombinations(occurrences: Occurrences, compNum: Int, list: List[Occurrences]): List[Occurrences] = {
    ...
    if (compNum < occurrences.length) {
      // Modify next letter if we have one
      newList = calcCombinations(occurrences, compNum + 1, newList)

      val tuple = occurrences.apply(compNum)
      // If we can reduce number of current letter - do so and findCombinations with reduced number
      if (tuple._2 > 0) {
  ...
  //type TreeLevel = List[(List[Word], TreeLevel)]
  ...
  def calcWordTree(occurrences: Occurrences): TreeLevel = {
    combinations(occurrences).map(comb => {
      ...
      if (comb != Nil) {
        ...
        if (words.nonEmpty) {
          // Если у предложения есть продолжение
          val subtractedOccurrences = subtract(occurrences, comb)
          if (subtractedOccurrences.nonEmpty) {
            // Если предложение ещё можно разворачивать дальше
            val leafs = calcWordTree(subtractedOccurrences)
            if (leafs != null) {
              level = (words, leafs.asInstanceOf[TreeLevel])
            }
          } else {
            // Если предложение завершилось естественным образом
            level = (words, null)
          }
        } else {
          // Если предложение неожиданно завершилось - оно не верно (1)
          level = null
        }
      }
      level
      ...
      // Убираем (1)
      .filter(_ != null)
      // Убираем c пустыми подуровнями. Подуровень пустой, если в нём были только (1)
