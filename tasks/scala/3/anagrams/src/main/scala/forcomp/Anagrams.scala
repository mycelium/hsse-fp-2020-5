...
object Anagrams {
 ...
 def wordOccurrences(w: Word): Occurrences = w.toLowerCase.map((_, 1)).groupBy(_._1)
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary groupBy wordOccurrecnces
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def loop(occurrences_list: Occurrences): List[Sentence] = {
      if (occurrences_list.nonEmpty) {
        for (combination <- combinations(occurrences_list)
             ...
             rest <- loop(subtract(occurrences_list, combination)))
          yield w :: rest
      }
      else {
        List(Nil)
